...
MongoClient.connect('mongodb://127.0.0.1:27017/ic', function(err, db) {
  if (err) throw err;
  var collection = db.collection('ic');
  getContract = function(id, callback) {
    console.log('get', id);
    return collection.findOne({ _id: new ObjectID(id) }, function(err, document) {
      document.vars = document.vars || {};
      document.sections = document.sections || [];
      if (err) throw err;
      callback && callback(document);
    });
  };
  putContract = function(id, data, callback) {
    console.log('put', id, data);
    return collection.update({ _id: new ObjectID(id) }, { $set: data }, { upsert: true }, function(err, document) {
...
app.get('/', function(req, res) {
  var defaults = {
    sections: [{ title: 'Blah blah blah', content: 'another blah blah' }],
    vars: {
      'title': 'Independent Contractor Services Agreement',
      'contractor': 'Imaginary, LLC',
      'contractor-address': 'Blah blah\nblahblah',
      'contractor-signer': 'Chris Roth',
      'company': 'Walk [Your City]',
      'company-signer': 'Matt Tomasulo, CEO',
      'company-address': 'Blah blah\nblahblah'
    }
  };
  putContract(undefined, defaults, function(contract) {
    res.redirect('/app/' + contract._id);
  });
});

app.get('/app/:id', function(req, res ) {
  getContract(req.params.id, function(contract) {
    contract.convertSection = convertSection;
    res.render('app', contract);
  ...
  phantom.create({ port: 1623, binary: config.phantomjs.binary }, function(ph) {
    ph.createPage(function(page) {
      page.set('paperSize', { format: 'A', orientation: 'portrait', border: '.5in' }, function (result) {
        page.open('http://localhost:' + config.port + '/contract', function(status) {
          console.log('wat');
...
app.post('/contract/:id', function(req, res) {
  putContract(req.params.id, req.body);
